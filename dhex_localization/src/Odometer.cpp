                          ...
                          const double dt)
{
    ...
    double omega = (dt * (right_velocity.data - left_velocity.data) / ((2*m_length)));
    m_pose.pose.pose.position.x = m_pose.pose.pose.position.x + (velocity) * (cos(theta + omega/2.));
    m_pose.pose.pose.position.y = m_pose.pose.pose.position.y + (velocity) * (sin(theta + omega/2.));
    theta+=omega;
    if (theta  > 2 * M_PI) {
        theta = theta  - 2*M_PI;
    } else if (theta  < 0){
